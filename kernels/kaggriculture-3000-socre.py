...
"""Replay-driven Kaggriculture policy with adaptive premium-sale timing."""
import base64
import copy
import json
import zlib
...
src = Path("submission.py").resolve()
submission = ROOT / "submission.py"
if src != submission.resolve():
    shutil.copy2(src, submission)

main = ROOT / "main.py"
shutil.copy2(submission, main)
...
compile(payload, "submission.py", "exec")
...
spec = importlib.util.spec_from_file_location("submission_module", submission)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)

assert callable(module.agent)
assert len(module._ROUTE) == 720
sample_action = {
    "farmer": ["PASS"],
    "hands": [],
    "market": [["SELL", "WHEAT", 2]],
}
assert module._final_drop_cash({}, sample_action, 717) == sample_action

# Final-turn DROP test.
hand_count = len(module._ROUTE[718].get("hands", []) or [])
inventories = [{} for _ in range(hand_count + 1)]

drop_index = next(
    i for i, order in enumerate(module._ROUTE[718]["hands"])
    if order and order[0] == "DROP"
)
inventories[drop_index + 1] = {"MILK": 4, "STRAWBERRY": 3}

hands_pos = [[0, 0] for _ in range(hand_count)]
hands_pos[drop_index] = [5, 4]
...
base = module._copy_plan(module._ROUTE[718])
patched = module._final_drop_cash(obs, base, 718)

assert ["SELL", "MILK", 4] in patched["market"]
assert ["SELL", "STRAWBERRY", 3] in patched["market"]
assert patched["market"][:2] == base["market"][:2]
